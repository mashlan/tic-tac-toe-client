...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  var passwordConfirmation = $('#sign-up-password-confirmation').val();
	...
	var onGoToChangePassword = function onGoToChangePassword() {
	  $('.jumbotron').hide();
	  $('.game-options').hide();
	  $('.account-change').show();
	  $('.nav').show();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var winLogic = __webpack_require__(14);
	var api = __webpack_require__(15);
	var ui = __webpack_require__(16);

	var linearBoard = new Array(9);
	...
	var onStartNewGame = function onStartNewGame(event) {
	  ...
	  api.createGame().then(ui.createSuccess).catch(ui.createFailure);
	};

	var createGameDelta = function createGameDelta(index, value, over) {
	...
	var onSelection = function onSelection(event) {
	  ...
	  if (gameDone) {} else if (linearBoard[index] === undefined) {
	    ...
	    api.updateGame(gameDelta).then(ui.updateSuccess).catch(ui.updateFailure);
	  }
	};

	var onGetGameStats = function onGetGameStats(event) {
	  event.preventDefault();
	  $('.alert').hide();
	  $('.authentication').hide();
	  $('.account-change').hide();
...
/***/ function(module, exports) {
	...
	var ticTacToe = function ticTacToe(tray, coords) {
	  return coords.map(function (c) {
	    return tray[c[0] * 3 + c[1]];
	  }).join('');
	};

	// function to loop through winLanes to create an array of 3 character
	// strings to check for wins or ties.
	var getLanes = function getLanes(tray) {
	  var lanes = [];
	  for (var i = 0; i < winLanes.length; i++) {
	    lanes.push(ticTacToe(tray, winLanes[i]));
	  }
	  return lanes;
	};

	var getWinner = function getWinner(tray) {
	  var lanes = getLanes(tray);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(11);
	var gameStats = __webpack_require__(17);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getTtlGamesFinished = function getTtlGamesFinished(games) {
	  var allGamesFinished = games.filter(function (e) {
	    return e.over;
	  });
	...
	var getTtlGamesAbandoned = function getTtlGamesAbandoned(games) {
	  var allGamesAbandoned = games.filter(function (e) {
	    return !e.over;
	  });
	...
	var getTtlGamesWon = function getTtlGamesWon(games) {
	  var allGamesWon = [];
	  allGamesWon = getTtlGamesFinished(games).filter(function (e) {
	    return winLogic.getWinner(e.cells);
	  });
	  return allGamesWon;
	};

	var getTtlGamesLost = function getTtlGamesLost(games) {
	  var allGamesLost = [];
	  allGamesLost = getTtlGamesFinished(games).filter(function (e) {
	    return !winLogic.getWinner(e.cells);
	  });
	  return allGamesLost;
	};

	var displayGameStats = function displayGameStats(games) {
